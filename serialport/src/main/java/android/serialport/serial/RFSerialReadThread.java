...
import android.serialport.serial.event.RFIDEvent;
...
public class RFSerialReadThread extends Thread {
    ...
    public RFSerialReadThread(InputStream is, String devicePath) {
    ...
    private void onDataReceive(final byte[] received, final int size) {
        ...
        while (temp < currentSize) {
            ...
            if (currentSize - temp >= Protocols.RF_MIN_SIZE) {
                // 第1个字节不为 43，则直接下一轮
                if (buffer[temp++] != Protocols.RF_HEAD_0) {
                    continue;
                }
                // 第2个字节不为 4D，则直接下一轮
                if (buffer[temp++] != Protocols.RF_HEAD_1) {
                    continue;
                }
                ...
                // 得到数据长度
                byte[] slenStr = ByteUtil.bytesToArr(buffer, frameStart + 4, 2);
                int slen = slenStr[0] + slenStr[1] * 256;

                // slen包括command和appdata，所以要把command的两字节减去
                //数据长度：最小长度  和  数据长度
                int total = Protocols.MIN_SIZE + slen;
                // 得到帧
                ...
                // 找到crc16校验位
                String ccb = ByteUtil.bytes2HexStr(buffer, frameEnd - 1, 1);
                // 除开CRC16的数据
                byte[] data = new byte[slen];
                // Header[2]	Command	Sequence	Len_Low	Len_High   Data[0…N]	BCC
                //   2 bytes	1 byte	1 bytes	     1 byte	 1 byte	   0~N bytes	1byte
                //  前面有6个字节 所以从第7个算起索引从0开始
                System.arraycopy(buffer, frameStart + 6, data, 0, data.length);
                // 计算其他数据的校验
                String otherCrc16 = ByteUtil.verify(data);
                //LogPlus.e("收到otherCrc16：" + otherCrc16);

                if (ccb.equals(otherCrc16)) {
                    //命令码字符串
                    String command = ByteUtil.bytes2HexStr(buffer, frameStart + 2, 1);
                    //数据
                    String appdata = ByteUtil.bytes2HexStr(buffer, frameStart + 6, slen);
                    // 所有数据
                    String allData = ByteUtil.bytes2HexStr(buffer, frameStart, total);

                    //LogPlus.e(devicePath+"收到==命令：" + ByteUtil.bytes2HexStr(buffer));
                    // 处理数据
                    EventBus.getDefault().post(new RFIDEvent(command, appdata, allData,devicePath));
